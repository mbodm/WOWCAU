...
namespace WOWCAU.Core.Parts.Modules.Addons.Defaults
{
    ...
    {
        private const string SmartUpdateFileName = "SmartUpdate.xml";

        private readonly ILogger logger = logger ?? throw new ArgumentNullException(nameof(logger));

        private readonly ConcurrentDictionary<string, SmartUpdateData> dict = new();

        public async Task LoadAsync(string workFolder, CancellationToken cancellationToken = default)
        {
            ...
            var xmlFile = Path.Combine(workFolder, SmartUpdateFileName);
        ...
        public async Task SaveAsync(string workFolder, CancellationToken cancellationToken = default)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(workFolder);
